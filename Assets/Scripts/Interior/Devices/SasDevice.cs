...
namespace Interior
{
    ...
    public class SasDevice : ShipDevice
    {
        ...
        /// оставляет как есть. Отклонённый стик снимает наведение: иначе пилот и автопилот тянули
        /// бы корабль в разные стороны, и на органах это выглядело бы как заедание, а не как спор.
        /// Стабилизацию он только прерывает: ей некуда тянуть, и отпущенный стик возвращает
        /// удержание — корабль гасит набранное вращение и встаёт там, куда его довернули.
        /// </summary>
        static void Steer(ref double axis, double value)
        {
            axis = value;
            if (value != 0.0 && !Holding()) Ship.orientation = ShipOrientation.Free;
        }
